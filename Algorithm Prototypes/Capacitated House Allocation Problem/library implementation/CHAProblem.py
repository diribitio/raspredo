...
class CHAProblem:
    ...
    def append_to_return_matching_state(self, name, A, H, E, M):
        self.return_values[name] = [{'agents': copy.deepcopy(A)}, {'houses': copy.deepcopy(H)},
                                    {'edges': copy.deepcopy(E)}, {'matchings': copy.deepcopy(M)}]
    ...
    def solve(self):
        ...
        for edge in remaining_E:
            transformed_E.append((edge[0], edge[1], self.preferences_lenght - edge[2],
                                  1))  # Max flow min cost algorithm -> weight must be small for higher prefernces (0 should represent the first choice)
        ...
        for loop_house in H:
            remaining_E.append(
                (loop_house.name, 'sink', 1, (loop_house.capacity - self.agents_in_house(loop_house, M))))
        ...
        if len(M) == len(copy_A):
            ...
            for copy_agent in copy_A:
                if len(self.fagents(copy_agent.preferences[0], copy_A)) > copy_agent.preferences[
                    0].capacity and self.agents_in_house(copy_agent.preferences[0], M) < copy_agent.preferences[
                    0].capacity and self.house_of_agent(copy_agent, M) != copy_agent.preferences[0]:
